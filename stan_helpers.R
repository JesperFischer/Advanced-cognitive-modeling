  ...
  for (t in 2:trials){
    ...
    for (ms in 1:(max(length(mean),length(sd)))){
      ...
      #lets fit in stan:
        ...
        df = sum %>% select("mean","variable") %>% filter(str_detect(variable, "bias") == T) %>% mutate(seed = seed, trials = t, priormean = mean1, priorsd = sd1)
...
parameter_recovery_rw_gen = function(ngames,trial,start_trials, end_trials, lr1_l, lr1_w,lr2_l,lr2_w){
  setwd("~/Advanced-cognitive-modeling/assignment2")
