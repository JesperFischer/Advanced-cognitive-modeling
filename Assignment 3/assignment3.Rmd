...
df %>% ggplot(aes(x = source1, y = outcome, col = as.factor(source2_dif)))+geom_line(position = "dodge")+facet_grid(kappa~bias)+theme_classic()
...
plot_prior_posterior_update(fit, parameters = c("bias","kappa","kappa2"), reals = c(bias = bias$bias, kappa = bias$kappa, kappa2 = bias$kappa2))



dfb %>% ggplot(aes(x = round(dfb$outcome_cat,0)))+geom_histogram()
...
inner_join(hier_simple[[3]],hier_simple[[2]]) %>% 
  ggplot(aes(x = source1, y = outcome, col = as.factor(source22)))+
  geom_line(position = "dodge")+facet_wrap(~name, nrow = 3, ncol = 4)+
  theme_classic()+
  geom_richtext(aes(x = 0.2, y = 0.8, label = paste0("bias = ",round(bias,2))), stat = "unique")+
  geom_richtext(aes(x = 0.2, y = 0.6, label = paste0("kappa = ",round(kappa,2))), stat = "unique")+
  geom_richtext(aes(x = 0.2, y = 0.4, label = paste0("kappa2 = ",round(kappa2,2))), stat = "unique")
```
...
plot_prior_posterior_update(fit, parameters, reals)
...
data.frame(pred = yrep[,]*9, real = y[,]) %>% pivot_longer(everything()) %>% ggplot(aes(x = value))+geom_histogram()+facet_wrap(~name, nrow = 2)+theme_classic()
...
df %>% ggplot(aes(x = source1, y = outcome, col = as.factor(source2_dif)))+geom_line(position = "dodge")+facet_grid(w1~w2,labeller = label_both)+theme_classic()
...
plot_prior_posterior_update(fit, parameters = c("bias","kappa","w1","w2","kappa2"), reals = c(bias = bias$bias, kappa = bias$kappa, w1 = bias$w1,w2 = bias$w2, kappa2 = bias$kappa2))


dfb %>% ggplot(aes(x = source1, y = outcome, col = as.factor(source2_dif)))+geom_line(position = "dodge")+facet_grid(kappa~bias)+theme_classic()
...
inner_join(hier_simple[[3]],hier_simple[[2]]) %>% 
  ...
  theme_classic()+
  geom_richtext(aes(x = 0.2, y = 0.8, label = paste0("bias = ",round(bias,2))), stat = "unique")+
  geom_richtext(aes(x = 0.2, y = 0.7, label = paste0("kappa = ",round(kappa,2))), stat = "unique")+
  geom_richtext(aes(x = 0.2, y = 0.6, label = paste0("kappa2 = ",round(kappa2,2))), stat = "unique")+
  geom_richtext(aes(x = 0.2, y = 0.5, label = paste0("w1 = ",round(w1,2))), stat = "unique")+
  geom_richtext(aes(x = 0.2, y = 0.4, label = paste0("w2 = ",round(w2,2))), stat = "unique")
...
elpd <- tibble(
  elpd_diff = loosimple$pointwise[, "elpd_loo"] - loo_weighted$pointwise[, "elpd_loo"]) %>% mutate(n = 1:nrow(.))
...
p2 = ggplot(elpd, aes(x = elpd_diff)) +
  ...
  theme_bw()+xlab("<--- weighted better   simple better --------->")
...
post %>% ggplot(aes(x = bias_mu))+geom_histogram()
```
